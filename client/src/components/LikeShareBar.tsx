...
export function LikeShareBar({ contentId }: LikeShareBarProps) {
  ...
  return (
    <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-sura-ivory/80">
      <button onClick={handleLike} className="rounded-full border border-sura-border/20 bg-sura-ink/80 px-3 py-2 transition hover:border-sura-gold/50 hover:text-sura-ivory">
        Like • {counts.likes}
      </button>
      <button onClick={handleShare} className="rounded-full border border-sura-border/20 bg-sura-ink/80 px-3 py-2 transition hover:border-sura-gold/50 hover:text-sura-ivory">
        Share • {counts.shares}
      </button>
    </div>
  );
