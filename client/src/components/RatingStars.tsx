...
export function RatingStars({ entityId, entityType }: RatingStarsProps) {
  ...
  return (
    <div className="space-y-2 text-sm text-sura-ivory/80">
      <div className="flex items-center gap-1">
        {stars.map((value) => (
          <button
            ...
            className={`text-xl transition ${userScore >= value ? 'text-sura-gold' : 'text-sura-ivory/40 hover:text-sura-gold'}`}
      ...
      <div className="text-xs text-sura-ivory/60">{`${count} ratings • ${average.toFixed(1)} avg`}</div>
    </div>
  );
}
