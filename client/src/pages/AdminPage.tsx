  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <header className="rounded-3xl border border-sura-ivory/10 bg-sura-ink/80 p-8">
        ...
        <p className="mt-3 text-sm leading-7 text-sura-ivory/80">
          {locale === 'ar'
      ...
      <section className="rounded-3xl border border-sura-ivory/10 bg-sura-ink/80 p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2 text-xs">
            <span className="rounded-full border border-sura-ivory/20 px-3 py-1 text-sura-ivory/80">
        ...
        <div className="space-y-3">
          {comments.length === 0 ? (
            <div className="text-sm text-sura-ivory/70">{locale === 'ar' ? 'لا توجد تعليقات.' : 'No comments found.'}</div>
          ) : (
            comments.map((comment) => (
              <article key={comment.id} className="rounded-2xl border border-sura-ivory/10 bg-black/20 p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="text-sm text-sura-ivory/85">
                    <span className="font-semibold">{comment.author || 'Reader'}</span>
                    <span className="mx-2">•</span>
                    <span>{comment.entityType}</span>
                    <span className="mx-2">•</span>
                    <span className="text-sura-ivory/70">{comment.entityId}</span>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => moderate(comment.id, 'visible')} className="rounded-full border border-sura-ivory/20 px-3 py-1 text-xs">
                      {locale === 'ar' ? 'إظهار' : 'Show'}
                    </button>
                    <button onClick={() => moderate(comment.id, 'hidden')} className="rounded-full border border-sura-ivory/20 px-3 py-1 text-xs">
                      {locale === 'ar' ? 'إخفاء' : 'Hide'}
                    </button>
                  </div>
                </div>
                <p className="mt-2 text-sm text-sura-ivory/85">{comment.message}</p>
                <div className="mt-2 text-xs text-sura-ivory/60">
