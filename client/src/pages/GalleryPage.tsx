    ...
    <div className="mx-auto max-w-7xl space-y-6">
        ...
        {(Array.isArray(items) ? items : []).map((item) => (
          <button key={item.id} onClick={() => setActive(item)} className="group overflow-hidden rounded-3xl border border-sura-ivory/10 bg-sura-dark/80 transition hover:-translate-y-1">
            ...
            <div className="p-4 text-left">
              <div className="text-xs uppercase tracking-[0.2em] text-sura-gold">{item.category}</div>
      ...
      {active && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-6">
          <div className="relative max-w-4xl overflow-hidden rounded-3xl bg-sura-dark p-6 shadow-2xl">
            <button onClick={() => setActive(null)} className="absolute right-4 top-4 rounded-full border border-sura-ivory/20 px-3 py-2 text-sm">Close</button>
            ...
            <div className="mt-4 text-center">
              ...
              <p className="mt-2 text-sm text-sura-ivory/70">{locale === 'ar' ? 'عرض كامل للقطعة المختارة.' : 'Full-screen view of the selected image.'}</p>
            </div>
          </div>
        </div>
      )}
