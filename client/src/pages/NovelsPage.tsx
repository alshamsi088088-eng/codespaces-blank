...
export function NovelsPage() {
  ...
  useEffect(() => {
    axios.get('/api/novels').then((res) => {
      ...
      setActiveNovel(res.data.novels[0] || null);
      setActiveChapter(res.data.novels[0]?.chapters?.[0] || null);
    });
  }, []);
        ...
        <aside className="space-y-4 rounded-3xl border border-sura-ivory/10 bg-black/30 p-6">
          ...
          {(Array.isArray(novels) ? novels : []).map((novel) => (
            <button
              ...
              className="block w-full rounded-3xl border border-sura-ivory/10 bg-sura-dark/80 px-4 py-4 text-left text-sm text-sura-ivory transition hover:border-sura-gold/60"
            >
        ...
        <article className={`rounded-3xl border border-sura-ivory/10 p-8 ${nightMode ? 'bg-sura-ink/80' : 'bg-sura-cream text-sura-brown'}`}>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="space-y-3 sm:flex sm:items-center sm:gap-3 sm:space-y-0">
              <button onClick={() => setNightMode((value) => !value)} className="rounded-full border border-sura-ivory/20 px-4 py-2 text-sm">
          ...
          <div className="mt-8 grid gap-3 rounded-3xl border border-sura-ivory/10 bg-black/20 p-4 sm:grid-cols-3">
            {(Array.isArray(activeNovel?.chapters) ? activeNovel?.chapters : []).map((chapter) => (
              <button
                ...
                className={`rounded-2xl p-4 text-left text-sm transition ${activeChapter?.id === chapter.id ? 'bg-sura-gold text-sura-dark' : 'bg-sura-dark/80 text-sura-ivory/80 hover:bg-sura-dark/100'}`}
              >
