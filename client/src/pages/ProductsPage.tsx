        ...
        {(Array.isArray(products) ? products : []).map((product) => (
          <article key={product.id} className="rounded-3xl border border-sura-ivory/10 bg-black/30 p-6">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-xs uppercase tracking-[0.3em] text-sura-gold">{product.type}</div>
            ...
            <p className="mt-4 text-sm leading-7 text-sura-ivory/80">{product.description}</p>
            <div className="mt-6 flex items-center justify-between gap-3">
              <span className="text-lg font-semibold text-sura-gold">${product.price.toFixed(2)}</span>
