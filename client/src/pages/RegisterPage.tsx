...
export function RegisterPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(locale === 'ar' ? 'حدث خطأ أثناء التسجيل.' : 'Registration failed.');
    }
  };
  ...
  return (
    <div className="mx-auto max-w-md space-y-6 rounded-3xl border border-sura-ivory/10 bg-sura-ink/80 p-8">
      ...
      <form onSubmit={submit} className="space-y-4">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder={locale === 'ar' ? 'الاسم الكامل' : 'Full name'} className="w-full rounded-3xl border border-sura-ivory/20 bg-sura-dark/80 px-4 py-3 text-sura-ivory" required />
        ...
        <input value={confirm} onChange={(e) => setConfirm(e.target.value)} type="password" placeholder={locale === 'ar' ? 'تأكيد كلمة المرور' : 'Confirm password'} className="w-full rounded-3xl border border-sura-ivory/20 bg-sura-dark/80 px-4 py-3 text-sura-ivory" required />
        <button type="submit" className="w-full rounded-full bg-sura-gold px-4 py-3 text-sm font-semibold text-sura-dark">{locale === 'ar' ? 'سجل' : 'Create account'}</button>
      </form>
