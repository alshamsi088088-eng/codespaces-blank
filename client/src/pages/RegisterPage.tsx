  ...
  return (
    <div className="mx-auto max-w-md space-y-6 rounded-3xl border border-sura-line bg-sura-canvas p-8">
      ...
      <form onSubmit={submit} className="space-y-4">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder={locale === 'ar' ? 'الاسم الكامل' : 'Full name'} className="w-full rounded-3xl border border-sura-line bg-sura-canvas px-4 py-3 text-sura-navy" required />
        ...
        <input value={confirm} onChange={(e) => setConfirm(e.target.value)} type="password" placeholder={locale === 'ar' ? 'تأكيد كلمة المرور' : 'Confirm password'} className="w-full rounded-3xl border border-sura-line bg-sura-canvas px-4 py-3 text-sura-navy" required />
